class Solution(object):
    def ladderLength(self, beginWord, endWord, wordList):
        ...
        begin_visit = set()
        begin_candidate = [beginWord]
        begin_num = 1

        end_visit = set()
        end_candidate = [endWord]
        end_num = 1

        while len(begin_candidate) > 0 and len(end_candidate) > 0:

            if len(begin_candidate) <= len(end_candidate) and len(begin_candidate) > 0:
                new_begin_num = 0
                for i in range(begin_num):
                    word = begin_candidate[i]
                    if word in end_candidate:
                        return level + 1

                    begin_visit.add(word)

                    for j in range(len(word)):
                        for k in range(26):
                            if alphbet[k] != word[j]:
                                nword = word[0:j]+alphbet[k]+word[j+1::]

                                if nword in wordSet and nword not in begin_visit:
                                    begin_visit.add(word)
                                    begin_candidate.append(nword)
                                    new_begin_num += 1

                begin_candidate = begin_candidate[begin_num::]
                begin_num = new_begin_num
                level += 1

            if len(begin_candidate) > len(end_candidate) and len(end_candidate) > 0:
                new_end_num = 0
                for i in range(end_num):
                    word = end_candidate[i]
                    if word in begin_candidate:
                        return level + 1
                    ...
                    for j in range(len(word)):
                        for k in range(26):
                            if alphbet[k] != word[j]:
                                ...
                                if nword in wordSet and nword not in end_visit:
                                    end_visit.add(word)
                                    end_candidate.append(nword)
                                    new_end_num += 1

                end_candidate = end_candidate[end_num::]
                end_num = new_end_num
                level += 1
    ...
    def build_graph(self, wordList):
        ...
        for i in range(n-1):
            ...
            for j in range(i+1, n):
                ...
                if self.near(word, nword):
                    counter[word].append(nword)
                    counter[nword].append(word)

        return counter

    def near(self, word, nword):
        diff = 0
...
if __name__ == '__main__':
    ...
    print demo.ladderLength(beginWord, endWord, wordList)
